...
- Registration no longer signs the administrator into the newly-created account; the new account establishes/links its authentication session on first login.
- Account creation and account-linking RPCs are no longer executable by anonymous clients.
...
## `supabase/migrations/20260926_harden_security_definer_grants.sql`
